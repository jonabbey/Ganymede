...
public class ADSyncMaster implements arlut.csd.ganymede.server.SyncMaster {
  ...
  public void augment(FieldBook book, DBEditObject eObj)
  {
    if (eObj.getTypeID() == SchemaConstants.UserBase)
      {
	if (eObj.isDefined(userSchema.EXCHANGESTORE) || eObj.getOriginal() != null && eObj.getOriginal().isDefined(userSchema.EXCHANGESTORE))
	  {
	    ...
	    if (exchangeStore != null)
	      {
		exchangeStore = (Invid) eObj.getOriginal().getFieldValueLocal(userSchema.EXCHANGESTORE);
	      }
