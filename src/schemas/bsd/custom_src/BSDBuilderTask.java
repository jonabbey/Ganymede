/*
   ...
   Last Mod Date: $Date: 1999/02/16 19:15:04 $
...
public class BSDBuilderTask extends GanymedeBuilderTask {
  ...
  public boolean builderPhase1()
  {
	...
	if (path == null)
	  {
	    throw new RuntimeException("BSDBuilder not able to determine output directory.");
	  }
	...
	try
	  ...
	  {
	    System.err.println("BSDBuilderTask.builderPhase1(): couldn't open group file: " + ex);
	  }
  ...
  public boolean builderPhase2()
  {
	...
	buildScript = buildScript + "bsd_builder";
      }
	...
	try
      ...
      {
	Ganymede.debug(buildScript + " doesn't exist, not running external BSD build script");
      }
  ...
  // ***
  //
  // The following private methods are used to support the BSD builder logic.
  //
  // ***
  ...
  /**
   *
   * This method writes out a line to the passwd BSD source file.
   *
  ...
  private void writeMasterUserLine(DBObject object, PrintWriter writer)
  {
    ...
    directory = (String) object.getFieldValueLocal(userSchema.HOMEDIR);
    shell = (String) object.getFieldValueLocal(userSchema.LOGINSHELL);
    classification = (String) object.getFieldValueLocal(userSchema.CLASSIFICATION);
  ...
  private void writeGroupLine(DBObject object, PrintWriter writer)
  {
    ...
    if (invids == null)
      {
	// System.err.println("BSDBuilder.writeUserLine(): null user list for group " + groupname);
      }
    else
      {
    ...
    if (result.length() > 1024)
      {
	System.err.println("BSDBuilder.writeGroupLine(): Warning!  group " + 
			   groupname + " overflows the BSD line length!");
      }
